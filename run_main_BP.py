...
parser.add_argument('--model', type=str, default='Time-LLM',
                    help='model name, options: [Autoformer, DLinear, Transformer]')
...
parser.add_argument('--total_pred_len', type=int, default=16, help='prediction sequence length')


# model define
parser.add_argument('--M_phi', type=int, default=0, help='phi as input')
parser.add_argument('--add_snr', type=int, default=0, help='add information of snr')
...
parser.add_argument('--llm_dim', type=int, default='1280', help='LLM model dimension')# LLama7b:4096; GPT2-small:768; BERT-base:768
...
for ii in range(args.itr):
    ...
    train_num_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    if accelerator.is_local_main_process:
        accelerator.print('model', args.model)
        accelerator.print('total_num_parameters',total_num_parameters)
        accelerator.print('train_num_parameters', train_num_parameters)
        accelerator.print('train_vs_total', train_num_parameters/total_num_parameters)
    trained_parameters = []
    ...
    for epoch in range(args.train_epochs):
        ...
        if args.is_training:
            ...
            for i, (batch_x_dict, batch_y_dict, batch_x_mark, batch_y_mark) in tqdm(enumerate(train_loader)):
                ...
                # TODO change input to phi and output to phi , then transfer phi to beam (use network)
                # TODO run Ke Ma's code.
                batch_x_snr= batch_x_dict['snr'].float().to(accelerator.device)
                batch_x = batch_x_dict['data'].float().to(accelerator.device)
                batch_y = batch_y_dict['data'].float().to(accelerator.device)
