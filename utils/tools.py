...
def get_near(y_gain, x_index,num_neighbor):
    B, T, N = y_gain.shape

    # 扩展x_index的维度
    x_index=torch.from_numpy(x_index).to(y_gain.device)

    # 创建一个用于存储邻居索引的张量
    neighbors = torch.zeros((B, T, 2 * num_neighbor + 1), dtype=torch.long, device=y_gain.device)

    # 计算每个索引的邻居
    for i in range(-num_neighbor, num_neighbor + 1):
        neighbors[:, :, i + num_neighbor] = (x_index + i) % N

    # 使用advanced indexing从y_gain中提取邻居的值
    y_gain_index=y_gain.argmax(dim=-1)
    neighbor_values = y_gain.gather(2, neighbors)
    # 找到邻居中最大值的索引
    max_indices = neighbor_values.argmax(dim=-1)+x_index-num_neighbor
    return max_indices
...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    ...
    model.eval()
    with torch.no_grad():
        for i, (batch_x_dict, batch_y_dict, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            if args.data=='BP':
                ...
                batch_x = batch_x_dict.float().to(accelerator.device)
                batch_y = batch_y_dict.float()
